...
namespace getbiz_work_project_management.Controllers
{
    ...
    public class ResourceAssignmentController : ControllerBase
    {
        ...
        public IActionResult GetTasksByResourceId(string Resource_Id, string Task_Id)
        {
            try
            {
                var messages = _resourceAssignmentReposiotry.GetTasksByResourceId(Resource_Id, Task_Id);
                if (messages == null)
                {
                    return NotFound();
                }
